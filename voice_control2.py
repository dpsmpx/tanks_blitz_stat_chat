...
Локальное распознавание речи (Vosk, офлайн).

Архитектура (потоки):
  main      — Tkinter mainloop + обновление оверлея (after)
  listen    — захват аудио с микрофона -> audio_queue
  recog     — audio_queue -> Vosk STT (локально) -> command_queue
  cmd       — command_queue -> обработка команд

Распознавание полностью офлайн (Vosk). Озвучка отключена.
"""
...
class AppConfig:
    ...
    export_file: str = "tank_battles.csv"    # Vosk модель
    vosk_model_name: str = "vosk-model-small-ru-0.22"
...
def download_vosk_model(cfg: AppConfig):
    """Скачивает и распаковывает модель Vosk при первом запуске."""
    model_path = os.path.join(cfg.vosk_model_dir, cfg.vosk_model_name)
        if os.path.exists(model_path):
        log.info("Модель Vosk уже загружена: %s", model_path)
        return model_path
    
    log.info("Модель Vosk не найдена. Начинаю скачивание...")
    log.info("URL: %s", cfg.vosk_model_url)
    
    zip_path = f"{cfg.vosk_model_name}.zip"
    
    try:
        # Скачивание
        log.info("Скачиваю модель (это может занять несколько минут)...")
        ...
        def report_progress(block_num, block_size, total_size):
            ...
            if total_size > 0:
                ...
                print(f"\rПрогресс: {percent:.1f}% ({mb_down:.1f}/{mb_total:.1f} МБ)", end="", flush=True)
        
        urllib.request.urlretrieve(cfg.vosk_model_url, zip_path, report_progress)
        print()  # Новая строка после прогресса
        
        # Распаковка
        log.info("Распаковываю модель...")
        os.makedirs(cfg.vosk_model_dir, exist_ok=True)
        
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            zip_ref.extractall(cfg.vosk_model_dir)
        
        # Удаление архива
        os.remove(zip_path)
        
        log.info("Модель успешно загружена и распакована: %s", model_path)
        return model_path
        
    except Exception as e:
        log.error("Ошибка при скачивании модели: %s", e)
        log.error("Проверьте подключение к интернету и попробуйте снова.")
        raise
...
def plural_ru(n: int, one: str, few: str, many: str) -> str:
    ...
    if n % 10 == 1 and n % 100 != 11:
        return one    if 2 <= n % 10 <= 4 and not (12 <= n % 100 <= 14):
        return few
    return many
...
class OverlayWindow:
    ...
    def _build_ui(self):
        c = self.cfg
        self.root.title("Танки Статистика")        self.root.overrideredirect(True)
...
class TankStatsRecorder:
    ...
    def _init_vosk(self):
        """Инициализация Vosk модели."""        try:
            model_path = download_vosk_model(self.cfg)
            log.info("Загружаю модель Vosk...")
            self.vosk_model = vosk.Model(model_path)
            self.vosk_recognizer = vosk.KaldiRecognizer(self.vosk_model, 16000)
            log.info("Модель Vosk готова к работе")
        except Exception as e:
            log.error("Не удалось инициализировать Vosk: %s", e)
            log.error("Распознавание речи не будет работать")
            self.vosk_model = None
    ...
    def _resume_session(self):
        for s in reversed(self.data["sessions"]):
            if s.get("end_time") is None:
                self.current_session = s                self.current_battle_damage = 0
    ...
    def _listen_worker(self):
        ...
        with mic as source:
            ...
            try:
                ...
                log.warning("Калибровка не удалась: %s", e)            log.info("Калибровка завершена. Слушаю.")
    ...
    def _recognition_worker(self):
        """Распознавание речи через Vosk (локально, офлайн)."""
        if not self.vosk_model:
            log.error("Vosk не инициализирован. Распознавание отключено.")
            return
        
        log.info("Распознавание Vosk активно")
        ...
        while not self.stop_event.is_set():
            ...
            try:
                # Конвертация AudioData в PCM для Vosk
                # Vosk ожидает: 16-bit, 16kHz, mono
                ...
                if self.vosk_recognizer.AcceptWaveform(raw_data):
                    ...
                    # Частичный результат (можно использовать для real-time отображения)
                    partial = json.loads(self.vosk_recognizer.PartialResult())
                    partial_text = partial.get("partial", "").strip()
                    if partial_text:
                        log.debug("Частично: %s", partial_text)
                        
            except Exception as e:                if not self.stop_event.is_set():
                    log.warning("Ошибка распознавания Vosk: %s", e)
    ...
    def process_command(self, raw: str):
        ...
        if has("экспорт", "выгрузка", "выгрузи"):
            self.export_csv()
            return        if has("итоги", "общая", "общий"):
            self.log_lifetime_stats()
            return
    ...
    def start_new_session(self, tank_name: str):
        ...
        self.current_battle_damage = 0
        self._save_data()
        log.info("Начата новая сессия для танка: %s", tank_name)
        self._update_overlay()

    def add_damage(self, damage: int):
        if not self.current_session:
            log.warning("Сначала начните новую сессию")
            return
        self.current_battle_damage += damage
        log.info("Урон %d. За бой: %d", damage, self.current_battle_damage)        self._update_overlay()

    def adjust_current_damage(self, delta: int):
        if not self.current_session:
            log.warning("Нет активной сессии")
            return
        self.current_battle_damage = max(0, self.current_battle_damage + delta)
        log.info("Урон за бой: %d", self.current_battle_damage)
        self._update_overlay()
    ...
    def undo_last_battle(self):
        ...
        if not s or not s.get("battles"):
            log.warning("Нет боёв для отмены")
            return
        last = s["battles"].pop()
        ...
        self._save_data()
        log.info("Последний бой отменён")        self.log_session_stats()
        self._update_overlay()
    ...
    def log_session_stats(self):
        ...
        n, v, dmg, wr, avg = self._session_stats(self.current_session)
        if n == 0:
            log.info("В сессии пока не было боёв")
            return
        best = self._best_damage(self.current_session)
        log.info(
            "Статистика сессии: %d %s, побед %d%%, средний урон %d, лучший бой %d",
            n, plural_ru(n, 'бой', 'боя', 'боёв'),
            wr, avg, best
        )
    ...
    def end_session(self):
        ...
        if n:
            log.info(
                "Сессия завершена. %d %s, побед %d%%, средний урон %d",
                n, plural_ru(n, 'бой', 'боя', 'боёв'), wr, avg
            )
        else:
            log.info("Сессия завершена. Боёв не было.")
        self.current_session = None
        self.current_battle_damage = 0
        self._save_data()
        self._update_overlay()

    def log_lifetime_stats(self):
        ...
        if n == 0:
            log.info("Нет данных %s", label)            return
        wr, avg = round(v / n * 100), round(dmg / n)
        log.info(
            "Итого %s: %d %s, побед %d%%, средний урон %d",
            label, n, plural_ru(n, 'бой', 'боя', 'боёв'), wr, avg
        )
    ...
    def export_csv(self):
        ...
        try:
            ...
            log.info("Статистика выгружена в файл")
        except Exception as e:
    ...
    def show_help(self):
        help_text = (
            ...
            "  статус                — статистика текущей сессии (в консоли)\n"
            "  итоги                 — статистика по танку за всё время (в консоли)\n"
            "  экспорт               — выгрузить всё в CSV\n"
            "  конец                 — завершить сессию\n"
            "  выход                 — закрыть программу\n"
            "  помощь                — эта справка\n"
            "Окно: перетаскивание ЛКМ, закрыть — Esc или Ctrl+Q.\n"
            "Все сообщения выводятся в консоль (озвучка отключена).\n"
        )
        print(help_text)
    ...
    def request_shutdown(self):
        ...
        self.stop_event.set()
        self._save_data()
        try:            self.overlay.root.after(0, self.overlay.shutdown)
        except Exception:
            pass

    def start(self):
        log.info("Программа готова. Скажите 'Помощь' для списка команд.")
